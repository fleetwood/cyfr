...
const ShrinkableIconLink = (props:ShrinkableIconLinkProps) => {
  const { icon, href, label, target, className, iconClassName, titleClassName, linkColor } = props;
  ...
  `}>
    <>
      {icon}
      {label && (<span className={`hidden lg:inline-block ${titleClassName||''}`}>{label}</span>)}
    </>
  </Link>
)};
