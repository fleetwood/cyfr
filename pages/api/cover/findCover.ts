...
const {info} = useDebug('api/cover/findCover')
...
export default async function handler(
  ...
  try {
    ...
    const covers = await PrismaCover.findCover(genre?.toString())
    if (!covers) {
      info('did not found', {genre})
    }
    res.status(200).json(covers) // return views as JSON
  } catch (error) {
    console.error(error)
